...
def rent_calculator(old_rent, new_rent, date: datetime, cpi_rates=cpi_rates):
    year = date.year
    month = date.month

    illegal_start = datetime(2020, 4, 1)
    illegal_end = datetime(2023, 10, 1)
    if illegal_start <= date <= illegal_end:
    ...
    if cpi_percent is None:
        return {"error": f"No CPI data available for this month"}
